...
describe('useTimeoutFn', () => {
  ...
  test('should return three functions', () => {
    // view - useTimeoutFn hook
    const view = renderHook(() => useTimeoutFn(() => {}, 5));

    expect(view.result.current.length).toBe(3);
    expect(typeof view.result.current[0]).toBe('function');
    expect(typeof view.result.current[1]).toBe('function');
    expect(typeof view.result.current[2]).toBe('function');
  });
  ...
  test('first function should return actual state of timeout', () => {
    let [, hook] = getHook();
  ...
  test('second function should cancel timeout', () => {
    const [spy, hook] = getHook();
  ...
  test('third function should reset timeout', () => {
    const [spy, hook] = getHook();
  ...
  test('should NOT reset timeout on function change', () => {
    const [spy, hook] = getHook(50);
